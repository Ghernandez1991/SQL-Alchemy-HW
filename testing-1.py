...
def start():
    ...
    for date in dict_temps1:
        search_term = date["start"].replace(" ", "").lower()
        Maximum_value1 = session.query(Measurement.tobs, func.max(Measurement.tobs)).filter(Measurement.date >= search_term).all()
        Minimum_value1 = session.query(Measurement.tobs, func.min(Measurement.tobs)).filter(Measurement.date >= search_term ).all()
        average_value1 = session.query(Measurement.tobs, func.avg(Measurement.tobs)).filter(Measurement.date >= search_term ).all()

        if search_term == canonicalized:
            return f"The max temp for that date is{Maximum_value1}, the min temp for that date is {Minimum_value1} and the avg temp for that date is {average_value1}"

    return jsonify({"error": f"Date with start date{start} not found."}), 404
