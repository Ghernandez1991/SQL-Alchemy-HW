...
def start():
    ...
    for date in dict_temps1:
        ...
        min_max_average = session.query(func.min(Measurement.tobs),func.max(Measurement.tobs),func.avg(Measurement.tobs)).filter(Measurement.station == 'USC00519281').group_by(Measurement.station).all()

        if search_term == canonicalized:
            return f"The min max and average  temp for that date is{min_max_average}"
